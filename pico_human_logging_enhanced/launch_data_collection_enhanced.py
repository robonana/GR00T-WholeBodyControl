...
class DataCollectionLaunchConfig:
    ...
    pico_zmq_port: int = 5556
    """Shared teleop PUB port used by the sidecar monitor."""
    ...
    # Raw PICO / human motion sidecar logging
    pico_human_monitor: bool = True
    """Run the independent raw PICO logger and quality monitor."""

    pico_raw_log_root_dir: str = ""
    """Empty uses <data-root-output-dir>/pico_human_logs."""

    pico_raw_record_continuous: bool = False
    """Record raw frames continuously instead of following PICO episode buttons."""

    pico_raw_chunk_size: int = 250
    """Raw frames per compressed NPZ chunk."""

    pico_monitor_console_output: bool = False
    """Periodic terminal metrics are opt-in; log writing is always enabled."""

    human_operator_id: str = "anonymous"
    human_height_m: float = 0.0
    pico_profile_height_m: float = 0.0
    human_arm_span_m: float = 0.0
    human_shoulder_width_m: float = 0.0
    human_inseam_m: float = 0.0
    human_thigh_length_m: float = 0.0
    human_shank_length_m: float = 0.0
    human_shoe_length_m: float = 0.0
...
SESSION_NAME = "sonic_data_collection_enhanced"
...
def main(config: DataCollectionLaunchConfig):
    ...
    dataset_name = config.dataset_name or datetime.now().strftime("%Y-%m-%d-%H-%M-%S")

    if config.pico_human_monitor and not config.pico_manager:
        raise ValueError("--pico-human-monitor requires --pico-manager")
    ...
    print(
        "  PICO raw log:    "
        f"{'Yes' if config.pico_human_monitor else 'No'} "
        f"(console={'on' if config.pico_monitor_console_output else 'off'})"
    )
    print("=" * 60)
    ...
    pico_cmd = (
        ...
        f"python pico_human_logging_enhanced/pico_manager_thread_server_enhanced.py "
        f"--input-source {config.pico_input_source} "
        f"--hand_mode {config.pico_hand_mode} "
        f"--port {config.pico_zmq_port}"
    )
    if config.pico_hand_mode.startswith("dh116s"):
    ...
    # --- Silent, independent human/PICO sidecar logger ---
    pico_log_dir = None
    if config.pico_human_monitor:
        pico_log_root = Path(config.pico_raw_log_root_dir) if config.pico_raw_log_root_dir else (
            Path(config.data_root_output_dir) / "pico_human_logs"
        )
        pico_log_dir = pico_log_root / dataset_name
        monitor_args = [
            str(repo_root / ".venv_teleop" / "bin" / "python"),
            "pico_human_logging_enhanced/pico_human_monitor.py",
            "--host", "localhost",
            "--port", str(config.pico_zmq_port),
            "--output-dir", str(pico_log_dir),
            "--dataset-name", dataset_name,
            "--chunk-size", str(config.pico_raw_chunk_size),
            "--operator-id", config.human_operator_id,
            "--operator-height-m", str(config.human_height_m),
            "--pico-user-height-m", str(config.pico_profile_height_m),
            "--arm-span-m", str(config.human_arm_span_m),
            "--shoulder-width-m", str(config.human_shoulder_width_m),
            "--inseam-m", str(config.human_inseam_m),
            "--thigh-length-m", str(config.human_thigh_length_m),
            "--shank-length-m", str(config.human_shank_length_m),
            "--shoe-length-m", str(config.human_shoe_length_m),
            "--pico-waist-mode", config.pico_waist_mode,
            "--pico-tracker-id", config.pico_tracker_id,
            "--notes", config.pico_log_notes,
        ]
        if config.pico_raw_record_continuous:
            monitor_args.append("--record-continuous")
        if config.pico_monitor_console_output:
            monitor_args.append("--console-output")
        monitor_cmd = (
            f"cd {shlex.quote(str(repo_root))} && {_shell_join(monitor_args)}"
        )
        subprocess.run(
            ["tmux", "new-window", "-d", "-t", SESSION_NAME, "-n", "pico_monitor"],
            check=True,
        )
        subprocess.run(
            ["tmux", "send-keys", "-t", f"{SESSION_NAME}:pico_monitor", monitor_cmd, "C-m"],
            check=True,
        )
        print(f"Starting silent PICO human logger (window: pico_monitor): {pico_log_dir}")
    ...
    exporter_cmd += f" --dataset-name {shlex.quote(dataset_name)}"
    exporter_cmd += (
    ...
    if pico_log_dir is not None:
        print(f"  PICO log dir: {pico_log_dir}")
    ...
    if config.pico_human_monitor:
        print("    Ctrl+b, n / p       - Switch to/from silent PICO monitor window")
